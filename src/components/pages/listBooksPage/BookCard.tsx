...
const BookCard: React.FC<BookCardProps> = ({ book, currentBorrows, currentReservations, context, renderLoanButton, renderReserveButton, bookInCurrentBorrows, handleDelete, handleEdit, activeAndLoanableReservations }) => {
    return (
        <Paper elevation={10} sx={{ padding: "2rem", width: { xs: "90%", md: "60%" } }} key={book.id}>
            <Stack
            sx={{
              display: "flex",
          ...
          >
            ...
            <Stack>
              ...
              <Button
                ...
                variant="contained"
                color="error"
                disabled={book.library_user !== context?.user?.id && !context?.user?.administrator}
